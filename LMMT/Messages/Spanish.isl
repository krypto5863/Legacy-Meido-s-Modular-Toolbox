...
SelectDirLabel3=Seleccione su directorio de instalación de CM3D2 a continuación. Ejemplo: C:\KISS\CM3D2 
SelectDirDesc=¿Dónde está ubicado tu juego? 

[CustomMessages]

;Startup Messages
ScamWarning=Si pagó por LMMT o lo descargó desde cualquier lugar que no sea la página oficial de GitHub, ¡puede ser que ha sido estafado o engañado!
StorageSpaceLow=Detectamos que su partición que contiene AppData (normalmente su disco C) no tiene suficiente espacio libre para el caché. Borre un mínimo de 5 GiB para instalar LMMT. Usted Tiene %1MBs
StorageSpaceLow2=La unidad que contiene la ruta que ha seleccionado no contiene suficiente espacio para instalar LMMT de forma segura. Borre un mínimo de 5 GiB para instalar LMMT en este directorio. Usted tiene %1MBs 
...
ReadmeExit=Regrese y léalo antes de instalar LMMT
...
CMDetected=¡Esto es COM3D2, no CM3D2! ¡LMMT no se hizo para COM3D2! Para COM3D2, utilice COM Modular Installer (CMI) 
NotaGameDir=¡Esto no parece ser un directorio CM3D2! Aún podemos continuar con la instalación, pero es posible que esté instalando en el directorio incorrecto, algunas funciones también se volverán ineficaces. ¿Seguimos de todos modos? 
ImproperUpdate=¡Hemos detectado que esta instancia del juego se actualizó incorrectamente! (Por lo general, mediante un proceso de arrastrar y soltar). Esto es completamente inseguro y PODRÁ romper el juego.%n%nDebido a que LMMT no se instalará, ya que puede encontrar más errores y errores si Seguir. Vuelve a instalar tu juego y actualiza CORRECTAMENTE (colocando los archivos de actualización / DLC en un directorio vacío y usando el update.exe o selector.exe provisto) para continuar.
IsInDownload=Parece que tu juego se encuentra en el directorio de descargas, ¡esto puede causar problemas con UAC y provocar instalaciones incorrectas de LMMT! Muévalo a un lugar más seguro (Ejemplo: C:/KISS/CM3D2) 
IsInProgramFiles=Parece que su juego está ubicado en el directorio Archivos de programa, ¡esto puede causar problemas con UAC y provocar instalaciones incorrectas de LMMT! Muévalo a un lugar más seguro (Ejemplo: C:/KISS/CM3D2)
IsINM=¡Hemos detectado INM! INM no es compatible con LMMT debido a diferencias técnicas.%n%nPara usar LMMT, instale el parche complementario de contenido para adultos/R18.%n%nSi no tiene una versión INM del juego, es probable que haya instalado Eng DLC/archivos en su juego japonés. Consulte el archivo readme sobre las instrucciones de reparación. 

GameOutdated=¡Tu juego está desactualizado! Actualícelo inmediatamente.%n%nVersión mínima:%1%n%nVersión encontrada:%2
GameUnsupported = ¡Tu juego tiene una versión que no es compatible! ¡Cambie de categoría o obtenga una versión de LMMT que sea compatible!%n%nVersión no admitida:%1 o más reciente%n%nVersión encontrada:%2 

MissingUpdateLst=Aunque parece ser una carpeta de juegos, ¡no hay ningún archivo Update.lst! ¡Esto es muy malo!%n%nActualiza su juego inmediatamente y asegúrese de que funciona antes de intentar instalar LMMT nuevamente.
...
CannotRenameOld=No se pudo cambiar el nombre de la carpeta OldInstall, pero la instalación ya está completa. Como resultado, no abortaremos ya que el error es inofensivo. Debería cambiarse el nombre automáticamente la próxima vez que se ejecute LMMT.
...
Survey=Abra la encuesta de comentarios para LMMT 