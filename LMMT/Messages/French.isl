...
SelectDirLabel3=Indiquer l'emplacement d'installation de CM3D2! Exemple: C:\KISS\CM3D2
SelectDirDesc=Où est installé votre jeu?

[CustomMessages]

;Startup Messages
ScamWarning=LMMT est totalement gratuit, si vous l'avez téléchargé ailleur que sir GitHub vous vous êtes probablement fait avoir!
...
StorageSpaceLow2=Espace insuffisant pour installer LMMT. Veulliez libérer au moins 5 GiBs: Vous avez %1MBs
...
CMDetected=Vous avez séléctionné COM3D2 et non CM3D2! LMMT n'est PAS compatible avec COM3D2! Veuillez utiliser COM3D2 Modular Installer (CMI)
NotaGameDir=Ceci n'est pas un dossier CM3d2 valide! Vous pouvez tout de même continuer, mais vous installez probablement dans le mauvais dossier. Certaines fonctions seront aussi innéfectives. Continuer?
ImproperUpdate=Ce jeu n'a pas été correctement mis à jour!(Habituelement par glissé déposé) Cela causera des problèmes.%n%nLMMT ne sera pas installé pour ne pas poser plus de problèmes. Veuillez réinstaller votre jeu et mettre à jour correctement (Extraire la mise à jour dans un dossier séparer et utiliser update.exe).
IsInDownload=Il semblerait que votre jeu soit installé dans le dossier téléchargement, cela peut poser quelques problèmes avec UAC et une mauvaise installation de LMMT! Veuillez le déplacer dans un emplacement adapté (Exemple: C:/KISS/CM3D2)
IsInProgramFiles=Il semblerait que votre jeu soit installé dans le dossier programmes, cela peut poser quelques problèmes avec UAC et une mauvaise installation de LMMT! Veuillez le déplacer dans un emplacement adapté (Exemple: C:/KISS/CM3D2)
IsINM=It's a NIght Magic détecté! INM n'est pas supporté par LMMT en raison des différence techniques etre les jeux.%n%nVeuillez installé le patch R18/Adult.%n%nSi vous n'êtes pas sur la version INM, vous avez probablement installé un DLC anglais sur votre version Japonaise, veuillez lire la documentation (en anglais) pour réparer ça.

GameOutdated=Votre jeu n'est pas à jour! %n%nVersion Minimum: %1%n%nVersion actuelle: %2
GameUnsupported=Cette version n'est pas suportée! Veuillez rétrograder ou utiliser une version de LMMT compatible!%n%nVersion non suportée:%1 ou plus récente%n%nVersion détectée:%2

MissingUpdateLst=Quoique correct ce dossier ne contient pas le fichier Update.lst! C'est mauvais signe!%n%nVeuillez faire une mise à jour à nouveau et vérifier que le jeu se lance avant d'installer LMMT.
...
MoveOldMods=Déplacer les anciens mods (seulement ceux de LMMT)
...
RemoveReadOnlyGroup=Il est courant de rencontrer des erreurs causées par des fichiers en lecture-seule, LMMT essaiera de corriger ça. Néanmoins certaines permission système peuvent l'en empècher, il faudra alors le faire manuellement

MoveOldExcep=Une erreur est survenue lors de la sauvegarde de l'ancienne instalation! Veuillez vous assurer que le jeu ne tourne pas et qu'un programme n'a accès aux dossiers du jeu. LMMT va quitter pour conserver vos données!
MoveOldModExcep=Une erreur est survenue lors de la sauvegarde des anciens mods! Veuillez vous assurer que le jeu ne tourne pas et qu'un programme n'a accès aux dossiers du jeu. LMMT va quitter pour conserver vos données!
CannotRenameOld=Le dossier de sauvegarde n'a pas put être renommé, cette erreur est sans conséquence et devrait être corrigée automatiquement la prochaine fois que LMMT sera lancé.
...
Survey=Ouvrir l'enquête satisfactiond de LMMT (en anglais)