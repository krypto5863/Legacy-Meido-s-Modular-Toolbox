    ...
    public class LMMTHelper
    {
...
#if DEBUG
                    System.Windows.Forms.MessageBox.Show("Directory Found!", "DEBUG", MessageBoxButtons.OK, MessageBoxIcon.Information);
                S   ystem.Windows.Forms.MessageBox.Show("Trying Directory Move!", "DEBUG", MessageBoxButtons.OK, MessageBoxIcon.Information);
#endif
        ...
        public static bool RenameOldInstall([MarshalAs(UnmanagedType.BStr)] string path)
        {
            if (Directory.Exists(path + @"\OldInstall"))
            {
                ...
                {
                    result = CMD.Run(@"rename " + '\u0022' + path + @"\OldInstall" + '\u0022' + " " + '\u0022' + @"OldInstall " + DateTime.Now.ToString("MM.dd.yyyy.h.mm.ss") + '\u0022');
                }
                else
                {
                    result = CMD.Run(@"rename " + '\u0022' + path + @"\OldInstall" + '\u0022' + " " + '\u0022' + @"OldInstall " + DateTime.Now.ToString("MM.dd.yyyy.h.mm") + '\u0022');
                }
...
#endif
            if (!File.Exists(path + @"\Mod\[CMI]GameData Link") && !Directory.Exists(path + @"\Mod\[CMI]GameData Link") && Directory.Exists(path + @"\Mod") && Directory.Exists(path + @"\Sybaris\GameData"))
            {
#if DEBUG
